...
- **音声付き動画生成 `/api/voiced-wan`** — 参照画像 + セリフ + シーン説明を指定すると、Irodori-TTS で音声生成 → 音声長から WAN I2V のフレーム数を自動逆算（`4n+1` スナップ）→ ffmpeg で MP4 合成。`voice_text` 省略時は LLM がシーンからセリフを自動生成
- **フルオート音声付き動画生成 `/api/auto-voiced-video`** — 日本語ひとことから MP4 まで全自動。LLM がシナリオ（Anima 構造化プロンプト＋セリフ）を生成 → AnimaBase v1.0 + ともぴ LoRA v6 で画像生成（832×480）と Irodori-TTS を **並列実行** → 音声長からフレーム数を逆算 → LightX2V NVFP4（4ステップ蒸留）で動画生成 → ffmpeg で音声付加
...
### LightX2V フルオート動画生成に必要なモデル・LoRA

```
~/infra/models/
├── Wan2.1-I2V-14B-480P/           # LightX2V ベースモデル（hf: Wan-AI/Wan2.1-I2V-14B-480P）
└── nvfp4/
    └── wan2.1_i2v_480p_nvfp4_lightx2v_4step.safetensors   # NVFP4 4ステップ蒸留チェックポイント（10.1GB）

~/infra/comfyui/models/
├── diffusion_models/
│   └── anima-base-v1.0.safetensors   # Anima UNET（画像生成に使用）
├── text_encoders/
│   └── qwen_3_06b_base.safetensors   # Anima 用テキストエンコーダー
├── vae/
│   └── qwen_image_vae.safetensors    # Anima 用 VAE
└── loras/
    └── tomopi_lora_v6_epoch20.safetensors → ~/infra/lora_train/model_v6/ （シンリンク）
```

セットアップ（初回のみ）:
```bash
bash ~/infra/setup_lightx2v.sh                  # LightX2V ビルド＆モデルDL
ln -sf ~/infra/lora_train/model_v6/tomopi_lora_v6_epoch20.safetensors \
       ~/infra/comfyui/models/loras/tomopi_lora_v6_epoch20.safetensors
```
...
└── ComfyUI-AnimaTool             # /anima/generate エンドポイントを ComfyUI に追加（フルオート動画生成に必要）
```
...
```bash
curl -X POST http://localhost:9000/api/auto-voiced-video \
  -H "Content-Type: application/json" \
  -d '{"message": "ともぴが笑顔で手を振る"}'
```

| パラメータ | デフォルト | 説明 |
|-----------|-----------|------|
| `message` | 必須 | 動画の内容を日本語で指示 |
| `fps` | 16 | フレームレート |
| `img_steps` | 25 | Anima 画像生成ステップ数 |
| `img_cfg` | 4.5 | Anima CFG スケール |
| `seed` | -1 | シード（-1=ランダム） |

**内部パイプライン**:
1. LLM → シナリオ `{tags, environment, dialogue}`（llama-server 停止）
2. `AnimaBase v1.0 + ともぴ v6 LoRA` で画像生成（832×480）‖ `Irodori-TTS` で音声生成 ← **並列**
3. 音声長 × fps → 最近傍 `4n+1` フレーム数
4. `LightX2V NVFP4`（4ステップ蒸留）で動画生成（MP4 直接出力）
5. `ffmpeg -c:v copy` で音声付加

**レスポンス**: `video_url`, `tags`, `environment`, `dialogue`, `frames`, `duration`, `seed`

完了後に `GET /api/voiced-wan-video?id=<job_id>` で MP4 をダウンロード。

### 音声付き動画生成（init_image 手動指定）

```bash
curl -X POST http://localhost:9000/api/voiced-wan \
  -H "Content-Type: application/json" \
  -d '{
    "init_image": "tomopi_test.png",
    "message": "anime girl, smiling, waving hand",
    "voice_text": "ねえ、一緒に遊ぼうよ！"
  }'
```

| パラメータ | デフォルト | 説明 |
|-----------|-----------|------|
| `init_image` | 必須 | ComfyUI input/ のファイル名または絶対パス |
| `message` | 必須 | WAN I2V のプロンプト（英語タグ推奨） |
| `voice_text` | — | セリフ（省略時は LLM が自動生成） |
| `fps` | 16 | フレームレート |
| `width` / `height` | 576 / 1024 | 解像度 |
| `steps` | 20 | WAN サンプリングステップ数 |
| `seed` | -1 | シード |
...
├── tts_wan_utils.py  # 音声付き動画生成パイプライン（Irodori-TTS + WAN/LightX2V + ffmpeg）
...
| `GET` | `/api/wan-i2v/progress/{prompt_id}` | Wan2.2 I2V の生成進捗確認 |
| `POST` | `/api/voiced-wan` | 参照画像 + セリフ + シーン説明 → Irodori-TTS + WAN I2V + ffmpeg → MP4 |
| `POST` | `/api/auto-voiced-video` | 日本語指示のみ → LLM シナリオ生成 → Anima 画像生成 ‖ TTS → LightX2V → MP4 (全自動) |
| `GET` | `/api/voiced-wan-video` | 生成済み音声付き MP4 を返す（`?id=<job_id>`） |
...
```
~/infra/comfyui/output/voiced_videos/<job_id>.mp4        # 最終 MP4（音声付き）
~/infra/comfyui/output/voiced_videos/<job_id>_raw.mp4    # LightX2V 出力（音声なし、フルオート版のみ）
~/infra/comfyui/output/auto_voiced/anima_frame_*.png     # Anima 生成画像（フルオート版のみ）
```
...
**フルオート動画生成で "/anima/generate route not found" エラー**
```bash
# ComfyUI-AnimaTool がインストールされているか確認
ls ~/infra/comfyui/custom_nodes/ComfyUI-AnimaTool/
# ComfyUI を再起動してルートを登録させる
~/infra/start-comfyui.sh
curl http://localhost:8188/anima/health
```

**フルオート動画生成で LightX2V エラー**
```bash
# LightX2V セットアップ確認
bash ~/infra/setup_lightx2v.sh
# LoRA シンリンク確認
ls -la ~/infra/comfyui/models/loras/tomopi_lora_v6_epoch20.safetensors
```

**音声生成（Irodori-TTS）が失敗する**
```bash
ls ~/infra/irodori-tts/.venv/bin/python
cd ~/infra/irodori-tts && uv sync --extra cu128
```
