...
### 8. 生成実験メモ（プロンプト実験記録）
**概要**: 生成履歴の各エントリに自由記述メモを紐付けて保存する。「このタグを追加したら塗りが変わった」等、プロンプト調整の試行錯誤を記録・再利用しやすくする。

**ユースケース**:
- タグ実験の観察記録（例: `anime coloring, cel shading` → 「アニメ塗りに近づいた」、`flat color hair` → 「髪のセル感が増した」）
- 良かった設定の理由や注意点をメモ
- 後で同じ方向性に戻りたいときの参照

**実装方針**:
- `comfy_chat_history` の各エントリに `memo` フィールドを追加
- 履歴グリッドのサムネイルクリックで開く詳細パネルにテキストエリアを追加
- `blur` / Enter で localStorage に即時保存（サーバーサイド変更不要）
- メモ付きエントリのサムネイルにアイコン（📝）を重ねて一覧で識別できるようにする

### 9. 実験メモの LLM コンテキスト注入（RAG 的活用）
**概要**: #8 で保存した実験メモを生成時に LLM のシステムプロンプトへ注入し、過去の試行錯誤をプロンプト生成に活かす。「アニメ塗りにしたい」と入力したとき、過去に記録した `anime coloring → アニメ塗りに近づいた` を LLM が参照して自動的にタグを選択する。

**前提**: #8（生成実験メモ）の実装が必要。

**実装方針（段階的）**:
- **フェーズ1 — 全件注入（メモ数 < 100件）**: `POST /api/generate` に `experiment_memos` フィールドを追加し、`comfy_utils.py` の `translate_prompt()` でシステムプロンプト末尾に全件テキストで注入。ベクトルDB 不要。Qwen3.5-9B unc のコンテキスト（40,960 トークン）に十分収まる。
- **フェーズ2 — キーワード検索（メモ数 ≥ 100件）**: ユーザー入力とメモテキストを単純照合し、関連メモだけ注入してトークン消費を抑える。
- **フェーズ3 — Embedding 検索（必要になったら）**: llama-server の `/v1/embeddings` でベクトル化し意味検索。「塗りを変えたい」→ `anime coloring` のメモがヒットするような意味的マッチが可能になる。

**変更ファイル（フェーズ1）**:
| ファイル | 変更内容 |
|---------|---------|
| `index.html` | `/api/generate` POST ボディに `experiment_memos` を追加 |
| `comfy_utils.py` | `translate_prompt()` のシステムプロンプト末尾にメモを注入 |
