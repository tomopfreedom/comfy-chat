...
VISION_HEALTH = "http://localhost:11435/health"
VISION_MODEL  = "mymodel-vision"
VISION_START  = os.path.expanduser("~/infra/start-llama-vision.sh")
...
_REVIEW_SYS = (
    "You are an expert evaluator of Stable Diffusion generated images. "
    "Evaluate the image using the provided user intent and positive prompt. "
    "Return JSON only — no other text.\n\n"
    "Evaluation criteria (listed in priority order):\n"
    "1. [TOP PRIORITY] Intent alignment: Does the image match the user's intended subject, scene, style, and atmosphere?\n"
    "2. [TOP PRIORITY] Prompt alignment: Are the elements described in the positive prompt visible in the image?\n"
    "3. Facial accuracy: asymmetry, distorted eyes/mouth, unnatural expression.\n"
    "4. Body proportions: distorted limbs, unnatural posture.\n"
    "5. Composition: subject cut off, unbalanced framing.\n"
    "6. Hands/fingers: report ONLY severe issues (7+ fingers, obvious fusing or missing limbs). Ignore minor imperfections.\n\n"
    "RULES:\n"
    "- positive_fix and negative_fix must be English tags only — no Japanese, no sentences.\n"
    "- Do NOT include in positive_fix any tag already present in the positive prompt.\n"
    "- Write issues and feedback in Japanese.\n"
    "- If issues is non-empty, at least one of positive_fix or negative_fix MUST be non-empty. "
    "For each issue identified, think about which Stable Diffusion tags would address it and include them.\n"
    "- If the user disputes or corrects a specific issue in your review, remove or revise that issue "
    "and update the score, feedback, positive_fix, and negative_fix accordingly. "
    "Always return the full updated JSON.\n\n"
    'Output format: {"score":1-10, "issues":["問題点（日本語）",...], '
    '"feedback":"総合コメント（日本語2-3文）", '
    '"positive_fix":"English tags to add to positive (comma-separated, empty string if none)", '
    '"negative_fix":"English tags to add to negative (comma-separated, empty string if none)"}'
)
...
async def review_image(
    ...
    """9B を停止して Vision LLM を起動し、画像をレビューして 9B を再起動する。"""
    subprocess.run(["systemctl", "--user", "stop", "llama-server"], check=False)
    subprocess.Popen([VISION_START])

    deadline = time.monotonic() + 90
    while time.monotonic() < deadline:
        await asyncio.sleep(3)
        try:
            async with session.get(VISION_HEALTH, timeout=aiohttp.ClientTimeout(total=3)) as r:
                if r.status == 200:
        ...
        raise RuntimeError("Vision モデルの起動タイムアウト（90秒）")

    try:
        async with session.get(
            base_url + image_url, timeout=aiohttp.ClientTimeout(total=30)
        ) as resp:
        ...
        if review_history:
            messages.extend(review_history)
        ...
        async with session.post(
            VISION_URL, json=payload, timeout=aiohttp.ClientTimeout(total=120)
        ) as resp:
            data = await resp.json(content_type=None)
    except asyncio.TimeoutError:
        raise RuntimeError("Vision モデルのレスポンスタイムアウト")
    finally:
